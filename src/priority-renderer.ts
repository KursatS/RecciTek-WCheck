...
import { SVG_EMPTY_STAR } from './utils/svgUtils'
const api = (window as any).electronAPI

// ── DOM Refs ─────────────────────────────────────────────────────────
const priorityList = document.getElementById('priority-list')!
const searchInput = document.getElementById('priority-search') as HTMLInputElement
...
function renderPriorityDevices(devices: any[]) {
    ...
    if (searchQuery) {
        filtered = filtered.filter(d =>
            (d.serial || '').toLowerCase().includes(searchQuery) ||
    ...
    filtered.forEach(device => {
        ...
        const timeStr = device.created_at
            ? new Date(device.created_at).toLocaleString('tr-TR')
            : ''
